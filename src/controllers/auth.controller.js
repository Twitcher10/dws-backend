...
async function verifyOtp(req, res, next) {
  try {
    ...
    if (userRes.rowCount === 0) return res.status(401).json({ message: "Invalid OTP" });
    ...
    if (expired) return res.status(401).json({ message: "OTP expired. Request a new one." });
    ...
    const resetToken = signResetToken({ id: u.id, email, purpose: "reset-password" });
...
async function resetPassword(req, res, next) {
  try {
    ...
    // Hash new password like login expects
    const password_hash = await bcrypt.hash(newPassword, 10);

    await query("UPDATE users SET password_hash=$1 WHERE id=$2", [password_hash, payload.id]);
